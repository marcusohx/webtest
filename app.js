...
app.get('/groupdevices/info/:id', authenticationMiddleware(), function(req,res){
	...
	db.query('SELECT deviceid, groupid, groupname,devicename from device_mapgroup WHERE groupid = ?',[req.params.id],function(error,results){
		if(error) throw error;
			...
			function(error,result){
				...
				if(result[0].countuser > 0){
					...
					bcrypt.hash(password, saltRounds, function(err, hash) {
						...
						db.query('INSERT INTO users(username,password,email,Cloudkey) VALUES(?,?,?,?)',[username,hash,email,nck],
...
app.get('/adddevice',authenticationMiddleware(),function(req,res){
	var sql1 = 'SELECT Cloudkey FROM users WHERE id = ?'
	db.query(sql1,[req.user],function(err,results){
		if(err) throw err;
...
app.post('/adddevice',function(req,res){
	...
	if(errors){
		...
		var sql1 = 'SELECT COUNT(*) AS countdevice FROM device WHERE devicename = ? AND id = ?'
		db.query(sql1,[username,userid],
			function(error,result){
				console.log(result[0].countdevice);
				if(result[0].countdevice > 0){
					console.log(result[0].countdevice);
					var sql3 = 'SELECT Cloudkey FROM users WHERE id = ?'
						db.query(sql3,[req.user],function(err,results3){
							if(err) throw err;
							res.render('adddevice',{
					...
					db.query(sql,[cloudkey,username,userid],
						function(error,results,fields){
							if(error){
...
app.post('/device/edit/:id',authenticationMiddleware(),function(req,res){
	var sql = 'UPDATE device SET Cloudkey = ?, devicename = ? WHERE	DeviceID = ?';
	var sql2 = 'UPDATE device_mapgroup SET Cloudkey = ?, devicename = ? WHERE	DeviceID = ?';
	var editdn = req.body.Editdevicename
	var editck = req.body.Editcloudkey
	db.query(sql,[editck,editdn,req.params.id],async function(error,results){
		if(error) throw error;
	...
	db.query(sql2,[editck,req.params.id],async function(error,results){
		if(error) throw error;
		if(results.affectedRows)
		{
			res.redirect('/devices')
...
app.get('/device/delete/:id',authenticationMiddleware(),function(req,res){
	var sql = 'DELETE FROM device WHERE DeviceID = ?'
	...
	if(devices == undefined || devices.length == 1){
		...
		db.query('SELECT groupid, groupname FROM mapgroup WHERE groupname = ? AND id = ?',[groupname,userid],
			function(error,results,fields){
				...
				db.query('SELECT CloudKey,devicename FROM device WHERE id = ?',[userid],
					function(error,result,fields){
						...
						for(i =0;i < devices.length; i++){
							db.query('INSERT INTO device_mapgroup(groupid,DeviceID,id,CloudKey,groupname,devicename) VALUES(?,?,?,?,?,?)',[gid,devices[i],userid,ck,gn,result[i].devicename],
								function(error,results,fields){
									if(error) throw error;
...
app.get('/groupdevice/delete/:id',authenticationMiddleware(),function(req,res){
	var sql = 'DELETE FROM device_mapgroup WHERE groupid = ?'
	var sql1 = 'DELETE FROM mapgroup WHERE groupid = ?'
	db.query(sql,[req.params.id],function(error,results){
		if(error) throw error;
	})
	db.query(sql1,[req.params.id],function(error,results){
		if(error) throw error;
		if(results.affectedRows)
		{
			res.redirect('/groupdevices')
...
app.get('/groupdevice/edit/:id',authenticationMiddleware(),function(req,res){
	db.query('SELECT deviceid, groupid, groupname,devicename from device_mapgroup WHERE groupid = ?',[req.params.id],function(error,results2){
		if(error) throw error;
...
app.post('/groupdevice/edit/:id', authenticationMiddleware(),function(req,res){
	...
	if(devices == undefined || devices.length == 1){
		...
		db.query('SELECT groupid, groupname FROM mapgroup WHERE groupid = ? AND id = ?',[req.params.id,userid],
			function(error,results,fields){
				if(error) throw error;
				var gid = results[0].groupid
				var gn = results[0].groupname
