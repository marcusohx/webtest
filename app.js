							...
							if(clientname !=  user){
								...
								db.query(sql3,[thegid],function(err,results){
									if(results[0].countrules > 0){ 
										...
										db.query(sql4,[thegid],function(err,results2){

											for(var i = 0;i < results.length;i++){
												var rs = results2[i].output.replace(/\s+/g,'');
												var currentsocket = clientname.split(',')
												console.log(currentsocket[0])
												console.log(results2[i].deviceid);
												if(cip === results2[i].input.replace(/\s+/g,'') && currentsocket[0] == results2[i].deviceid){
													console.log(rs);
